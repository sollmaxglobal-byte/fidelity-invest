  ...
  useEffect(() => {
    ...
    const notify = (kind: "deposit" | "withdrawal", row: StatusRow) => {
      ...
      if ("Notification" in window && Notification.permission === "granted") {
        new Notification("Fidelity", { body: message, icon: "/fidelity-app-icon-192.png", tag: `${kind}-${row.id}-${row.status}` });
      }
    };
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "deposits", filter: `user_id=eq.${user.id}` }, (payload) => notify("deposit", payload.new as StatusRow))
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "withdrawals", filter: `user_id=eq.${user.id}` }, (payload) => notify("withdrawal", payload.new as StatusRow))
      .subscribe();
