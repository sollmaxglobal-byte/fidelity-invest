    ...
    try {
      if (on) {
        ...
        if (res === "enabled") {
          ...
          toast.error(fr ? "Notifications bloquées par le navigateur" : "Notifications blocked in your browser");
        }
      }
    } catch {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <Button size="sm" variant={on ? "outline" : "default"} disabled={busy || !supported} onClick={toggle}>
          {!supported ? (fr ? "Non pris en charge" : "Unsupported") : on ? (fr ? "Désactiver" : "Turn off") : (fr ? "Activer" : "Turn on")}
        </Button>
      </div>
