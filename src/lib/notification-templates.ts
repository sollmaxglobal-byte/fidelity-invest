...
export const adminRequestNotification = {
  deposit: (input: { name: string; email: string; amount: string; method: string; transactionId: string; reviewUrl: string }): AdminRequestNotification => ({
    subject: `New deposit request · ${input.amount}`,
    text: `A new deposit request requires review.\n\nInvestor: ${input.name}\nEmail: ${input.email}\nAmount: ${input.amount}\nMethod: ${input.method}\nReference: ${input.transactionId}\nReview: ${input.reviewUrl}`,
    variables: { ...input },
  }),
  withdrawal: (input: { name: string; email: string; amount: string; method: string; account: string; transactionId: string; reviewUrl: string }): AdminRequestNotification => ({
...
export const adminSmsTemplates = {
  en: {
    deposit: "Fidelity: New deposit request from {{name}} for {{amount}} via {{method}}. Ref {{transaction_id}}. Review: {{review_url}}",
    withdrawal: "Fidelity: New withdrawal request from {{name}} for {{amount}} via {{method}}. Ref {{transaction_id}}. Review: {{review_url}}",
  },
  fr: {
    deposit: "Fidelity : Nouvelle demande de dépôt de {{name}} pour {{amount}} via {{method}}. Réf. {{transaction_id}}. Vérifier : {{review_url}}",
    withdrawal: "Fidelity : Nouvelle demande de retrait de {{name}} pour {{amount}} via {{method}}. Réf. {{transaction_id}}. Vérifier : {{review_url}}",
  },
};
