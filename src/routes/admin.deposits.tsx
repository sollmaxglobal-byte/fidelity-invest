    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Review and approve incoming deposits.</p>
        </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 text-xs text-muted-foreground">
                    {d.profiles?.full_name ?? "User"} • {d.profiles?.phone ?? "—"} • {formatDate(d.created_at)}
                  </div>
                  <div className="mt-2 text-sm">
                    <span className="text-muted-foreground">Method:</span> {d.payment_methods?.label ?? "—"}
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {d.status === "pending" ? (
                    ...
                    <span className={`rounded-full px-2.5 py-1 text-xs font-medium capitalize ${
                      d.status === "approved" ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"
                    }`}>
                      {d.status}
                    </span>
                  )}
