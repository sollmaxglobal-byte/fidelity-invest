...
export const Route = createFileRoute("/dashboard/referrals")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Track everyone you referred to Fidelity and the investment plan they are running." },
      { property: "og:title", content: "My referrals — Fidelity" },
      { property: "og:description", content: "See your referral team and their active investment plans." },
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl border border-border bg-card p-4">
          ...
          <div className="mt-1 font-display text-2xl font-bold uppercase tabular-nums text-primary">{rows.length}</div>
        </div>
        <div className="rounded-2xl border border-border bg-card p-4">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">With active plan</div>
          <div className="mt-1 font-display text-2xl font-bold uppercase tabular-nums text-success">{active}</div>
        </div>
      </div>
          ...
          {rows.map((r, i) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="font-display text-base text-primary">{r.full_name || "Member"}</div>
                  {r.joined_at && (
                    <div className="text-xs text-muted-foreground">Joined {formatDate(r.joined_at)}</div>
                  )}
                </div>
                <span
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 border-t border-border pt-3">
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Investment plan</div>
