...
export const Route = createFileRoute("/dashboard/receipt/$kind/$id")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Official Fidelity transaction receipt with full payment details and transaction ID." },
...
function ReceiptPage() {
  ...
  if (loading) return <div className="py-20 text-center text-sm text-muted-foreground">Loading receipt…</div>;
  if (!row)
    return (
      <div className="space-y-4 py-16 text-center">
        <p className="text-sm text-muted-foreground">Receipt not found.</p>
        <Link to="/dashboard/wallet" className="text-sm font-medium text-primary underline">Back to wallet</Link>
      </div>
    );
  ...
  const toneBg =
    meta.tone === "success" ? "bg-success/15 text-success border-success/30"
